...
import filemanager.fileexplorer.provider.CloudStorageProvider;
import filemanager.fileexplorer.provider.DocumentsProvider;
import filemanager.fileexplorer.provider.ExternalStorageProvider;
import filemanager.fileexplorer.provider.ExtraDocumentsProvider;
import filemanager.fileexplorer.provider.MediaDocumentsProvider;
import filemanager.fileexplorer.provider.NetworkStorageProvider;
import filemanager.fileexplorer.provider.NonMediaDocumentsProvider;
import filemanager.fileexplorer.provider.RecentsProvider;
import filemanager.fileexplorer.provider.RootedStorageProvider;
...
import filemanager.fileexplorer.cloud.CloudConnection;
import filemanager.fileexplorer.libcore.io.IoUtils;
import filemanager.fileexplorer.libcore.io.MultiMap;
...
public class RootsCache {
    ...
    private class UpdateTask extends AsyncTask<Void, Void, Void> {
        ...
        protected void onPostExecute(Void aVoid) {
            ...
            mContext.sendBroadcast(new Intent(HomeFragment.ROOTS_CHANGED));
        }
    }
    ...
    public ArrayList<RootInfo> getShortcutsInfo(){
        ...
        for (RootInfo root : mRoots.get(NonMediaDocumentsProvider.AUTHORITY)) {
            final boolean empty = (root.flags & DocumentsContract.Root.FLAG_EMPTY) != 0;
            if (!empty) {
                list.add(root);
            }
        }
        list.add(getDownloadRoot());
        return list;
    }
    ...
    static List<RootInfo> getMatchingRoots(Collection<RootInfo> roots, BaseActivity.State state) {
        ...
        for (RootInfo root : roots) {
            ...
            if(null != root.authority
            	...
            	if(state.action != BaseActivity.State.ACTION_BROWSE || !state.rootMode){
            		continue;
            	}
            }
            // Exclude read-only devices when creating
            if (state.action == BaseActivity.State.ACTION_CREATE && !supportsCreate) continue;
            // Exclude roots that don't support directory picking
            if (state.action == BaseActivity.State.ACTION_OPEN_TREE && !supportsIsChild) continue;
            // Exclude advanced devices when not requested
            if (!state.showAdvanced && advanced) continue;
            // Exclude non-local devices when local only
            if (state.localOnly && !localOnly) continue;
            // Only show empty roots when creating
            if (state.action != BaseActivity.State.ACTION_CREATE && empty) continue;

            if ((state.action == BaseActivity.State.ACTION_GET_CONTENT
                    || state.action == BaseActivity.State.ACTION_GET_CONTENT || state.action == BaseActivity.State.ACTION_OPEN
                    || state.action == BaseActivity.State.ACTION_OPEN_TREE) && superAdvanced){
                continue;
            }
