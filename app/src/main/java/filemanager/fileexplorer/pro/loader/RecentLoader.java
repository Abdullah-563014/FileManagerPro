...
package filemanager.fileexplorer.pro.loader;
...
import filemanager.fileexplorer.pro.cursor.FilteringCursorWrapper;
import filemanager.fileexplorer.pro.cursor.RootCursorWrapper;
import filemanager.fileexplorer.pro.cursor.SortingCursorWrapper;
import filemanager.fileexplorer.pro.libcore.io.IoUtils;
import filemanager.fileexplorer.pro.misc.AsyncTaskLoader;
...
public class RecentLoader extends AsyncTaskLoader<DirectoryResult> {
    ...
    private static final String[] RECENT_REJECT_MIMES = new String[] { DocumentsContract.Document.MIME_TYPE_DIR };
    ...
    private final int mSortOrder = BaseActivity.State.SORT_ORDER_LAST_MODIFIED;
    ...
    public RecentLoader(Context context, RootsCache roots, BaseActivity.State state) {
        super(context);
        mRoots = roots;
        mState = state;
    ...
    public DirectoryResult loadInBackground() {
        if (mFirstPassLatch == null) {
            ...
            for (RootInfo root : roots) {
                if ((root.flags & DocumentsContract.Root.FLAG_SUPPORTS_RECENTS) != 0) {
        ...
        if (LOGD) {
            Log.d(BaseActivity.TAG, "Found " + cursors.size() + " of " + mTasks.size() + " recent queries done");
        }
    ...
    public class RecentTask extends FutureTask<Cursor> implements Runnable, Closeable {
        ...
        private synchronized void runInternal() {
            ...
            try {
                ...
                Log.w(BaseActivity.TAG, "Failed to load " + authority + ", " + rootId, e);
            } finally {
