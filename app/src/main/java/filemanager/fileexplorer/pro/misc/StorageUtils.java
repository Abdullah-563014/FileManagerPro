        ...
        for (Object object : vi) {
            ...
            if(Utils.hasPie() && !TextUtils.isEmpty(path)) {
                id = TextUtils.isEmpty(id) && !TextUtils.isEmpty(path)
                        ? (path.contains(VolumeInfo.ID_EMULATED_INTERNAL) ? VolumeInfo.ID_EMULATED_INTERNAL : "") : id;
                if(TextUtils.isEmpty(id)){
                    if(!first){
                        first = true;
                        id = VolumeInfo.ID_EMULATED_INTERNAL;
                    }
                }
            }
...
/*        if (!TextUtils.isEmpty(vol.fsUuid)) {
        ...
        if (vol.disk != null) {
            ...
            } else if ((flags & DiskInfo.FLAG_USB) != 0) {
                if (vol.disk.isInteresting(label)) {
